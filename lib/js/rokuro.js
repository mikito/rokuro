...
(function() {
  ...
  var onHandDetectEvent = function() {};
  ...
  Rokuro.initialize = function(callbacks) {
    ...
    onHandDetectEvent = function(handCount) {
      if(handsDetected != handCount) {
        console.log("handCount Updated: " + handCount);
        callbacks.onHandDetect(handCount);
  ...
  Rokuro.recognize = function(hands) {
    if(hands) {
      onHandDetectEvent(hands.length);
